...
class StairMeshCreator {
	public static function create(x:Float, y:Float, z:Float, yaw:Float):Mesh {
		...
		addStepWall(0, 0.50, 0.75, surf);
		addStepWall(0, 0.75, 1, surf);
		addStepFloor(0, 0.0, 0, surf);
		addStepFloor(0, 0.25, 0.25, surf);
		addStepFloor(0, 0.50, 0.50, surf);
		addStepFloor(0, 0.75, 0.75, surf);
	...
	private static function addStepWall(x:Float, y:Float, z:Float, surf:GridSurface):Void {
		...
		final sy = y - 0.5;
		final sz = z - 0.5;
		final a = surf.addVertex(new GridVertex(sx, sy, sz, 0, 0, -1, Cs.COLOR_WHITE, 0, 0.25));
		final b = surf.addVertex(new GridVertex(sx + 1, sy, sz, 0, 0, -1, Cs.COLOR_WHITE, 1, 0.25));
		final c = surf.addVertex(new GridVertex(sx + 1, sy + 0.25, sz, 0, 0, -1, Cs.COLOR_WHITE, 1, 0));
		final d = surf.addVertex(new GridVertex(sx, sy + 0.25, sz, 0, 0, -1, Cs.COLOR_WHITE, 0, 0));
		surf.addIndex(a);
		surf.addIndex(d);
		surf.addIndex(c);
		surf.addIndex(a);
