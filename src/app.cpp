...
void draw_texture_ex(ITexture* tex, int x, int y, int w, int h, u32 color) {
    if (!tex) return;
    SColor colors[4];
    for (int i = 0; i < 4; ++i) colors[i] = SColor(color);
...
void set_material_type(SMaterial& mat, int type) {
    switch (type) {
        case MATERIAL_SOLID:        mat.MaterialType = EMT_SOLID; break;
        case MATERIAL_SOLID_ALPHA:  mat.MaterialType = EMT_TRANSPARENT_ADD_COLOR; break;
        case MATERIAL_LIGHTMAP:     mat.MaterialType = EMT_LIGHTMAP; break;
        case MATERIAL_LIGHTMAP_ADD: mat.MaterialType = EMT_LIGHTMAP_ADD; break;
        case MATERIAL_LIGHTMAP_MUL: mat.MaterialType = EMT_LIGHTMAP_M2; break;
        case MATERIAL_LIGHTMAP_MUL4:mat.MaterialType = EMT_LIGHTMAP_M4; break;
        case MATERIAL_ALPHA:        mat.MaterialType = EMT_TRANSPARENT_ALPHA_CHANNEL; break;
        case MATERIAL_VERTEXALPHA:  mat.MaterialType = EMT_TRANSPARENT_VERTEX_ALPHA; break;
        case MATERIAL_ALPHATEST:    mat.MaterialType = EMT_TRANSPARENT_ALPHA_CHANNEL_REF; break;
        case MATERIAL_SPHERE:       mat.MaterialType = EMT_SPHERE_MAP; break;
        case MATERIAL_REFLECT:      mat.MaterialType = EMT_REFLECTION_2_LAYER; break;
        case MATERIAL_ONETEX:       mat.MaterialType = EMT_DETAIL_MAP; break;
        default: break;
    }
}
...
void set_material_flag(SMaterial& mat, int flag, bool enable) {
    switch (flag) {
        case FLAG_LIGHTING:        mat.setFlag(EMF_LIGHTING, enable); break;
        case FLAG_FOG:             mat.setFlag(EMF_FOG_ENABLE, enable); break;
        case FLAG_ZREAD:           mat.setFlag(EMF_ZBUFFER, enable); break;
        case FLAG_ZWRITE:          mat.setFlag(EMF_ZWRITE_ENABLE, enable); break;
        case FLAG_BACKFACECULLING: mat.setFlag(EMF_BACK_FACE_CULLING, enable); break;
        case FLAG_NORMALIZE:       mat.setFlag(EMF_NORMALIZE_NORMALS, enable); break;
        case FLAG_VERTEXCOLORS:
            mat.ColorMaterial = enable ? ECM_DIFFUSE_AND_AMBIENT : ECM_NONE;
            break;
        case FLAG_CLAMP:           mat.setFlag(EMF_TEXTURE_WRAP, enable); break;
        default: break;
    }
}
