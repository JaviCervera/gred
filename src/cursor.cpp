...
Cursor::Cursor(Grid &g) : grid(g), mesh(nullptr), alpha(0.5f), alpha_dir(1.f), tile_height_(1)
{
...
void Cursor::update(bool editing)
{
    ...
    if (App::key_hit[KEY_UP])
        App::translate_entity(entity, 0.f, 0.f, 1.f);
    if (App::key_hit[KEY_DOWN])
        App::translate_entity(entity, 0.f, 0.f, -1.f);
    if (App::key_hit[KEY_LEFT])
        App::translate_entity(entity, -1.f, 0.f, 0.f);
    if (App::key_hit[KEY_RIGHT])
        App::translate_entity(entity, 1.f, 0.f, 0.f);
    if (App::key_hit[KEY_KEY_Q])
        App::translate_entity(entity, 0.f, 1.f, 0.f);
    if (App::key_hit[KEY_KEY_A])
        App::translate_entity(entity, 0.f, -1.f, 0.f);
    if (App::key_hit[KEY_KEY_J])
        tile_height(tile_height() - 1);
    if (App::key_hit[KEY_KEY_K])
        tile_height(tile_height() + 1);
...
uint8_t Cursor::tile_height() const
{
    return tile_height_;
}

void Cursor::tile_height(uint8_t height)
{
    tile_height_ = std::clamp(height, uint8_t(1), uint8_t(8));
}
